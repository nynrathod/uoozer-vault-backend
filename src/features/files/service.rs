...
impl FileService {
    ...
    pub async fn create_file(
        ...
        const USER_STORAGE_QUOTA_BYTES: i64 = 100 * 1024 * 1024; // 100MB POC limit
        ...
        if let Some(folder_id) = req.folder_id {
            Self::verify_folder_ownership(&self.db, folder_id, user_id).await?;
        }

        // ── Dedup check: same-user, same plaintext_blake3, active version ──
        ...
        // ── No dedup: create new file + version + chunks ──
        ...
        sqlx::query(
            "INSERT INTO files
                (file_id, user_id, folder_id, encrypted_metadata, metadata_nonce,
                 plaintext_blake3, total_size, current_version_id)
        ...
        sqlx::query(
            "INSERT INTO file_versions
                (version_id, file_id, version_number, encryption_header,
                 total_size, total_chunks, plaintext_blake3, created_by_device_id, is_active)
             VALUES ($1, $2, 1, $3, $4, $5, $6, $7, false)",
        )
        .bind(version_id)
        .bind(file_id)
        .bind(&encryption_header)
        .bind(req.total_size)
        ...
        for chunk in &req.chunks {
            ...
            sqlx::query(
                "INSERT INTO file_chunks
                    (version_id, chunk_index, segment_index, chunk_size, chunk_blake3, r2_key)
                 VALUES ($1, $2, $3, $4, $5, $6)",
            )
            .bind(version_id)
        ...
        let upload_urls = self
            .generate_upload_urls(r2, user_id, file_id, version_id, &req.chunks)
            .await?;
    ...
    pub async fn create_version(
        ...
        // ── Version dedup: if plaintext_blake3 matches current active version, skip ──
        let current_version: Option<Uuid> = sqlx::query_scalar(
            "SELECT v.version_id
             FROM file_versions v
             WHERE v.file_id = $1 AND v.is_active = true AND v.plaintext_blake3 = $2",
        )
        .bind(file_id)
        .bind(&plaintext_blake3)
        .fetch_optional(&self.db)
    ...
    pub async fn list_files(
        ...
        let total: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM files
             WHERE user_id = $1 AND folder_id IS NOT DISTINCT FROM $2 AND deleted_at IS NULL",
        )
        .bind(user_id)
        .bind(folder_id)
        .fetch_one(&self.db)
    ...
    pub async fn update_file(
        ...
        // If both metadata fields provided, both must be present
        ...
        // Verify new folder ownership if moving
        if let Some(Some(new_folder_id)) = req.folder_id {
            Self::verify_folder_ownership(&self.db, new_folder_id, user_id).await?;
        }
    ...
    pub async fn get_download_manifest(
        ...
        // Use requested version or current active version
        let target_version_id = match version_id {
            Some(vid) => {
                // Verify this version belongs to this file and user
                let exists: Option<(Uuid,)> = sqlx::query_as(
                    "SELECT v.version_id
                     FROM file_versions v
        ...
        let version: (Vec<u8>, i64, i32) = sqlx::query_as(
            "SELECT encryption_header, total_size, total_chunks
        ...
        let chunks: Vec<(i32, i32, i64, String)> = sqlx::query_as(
            "SELECT chunk_index, segment_index, chunk_size, r2_key
        ...
        for (chunk_index, segment_index, chunk_size, r2_key) in chunks {
            let url = r2.presign_get(&r2_key).await?;
            chunk_infos.push(DownloadChunkInfo {
                chunk_index,
                segment_index,
                chunk_size,
                presigned_url: url,
    ...
    pub async fn restore_version(
        ...
        // Verify ownership + version belongs to file
        let version: Option<(Uuid, bool)> = sqlx::query_as(
            "SELECT v.version_id, v.is_active
             FROM file_versions v
             JOIN files f ON f.file_id = v.file_id
    ...
    async fn validate_create_request(&self, req: &CreateFileRequest) -> Result<(), AppError> {
        ...
        // secretstream_xchacha20poly1305 adds exactly 17 bytes of overhead per chunk.
        // If this math doesn't line up, the client is lying about sizes or using broken crypto.
